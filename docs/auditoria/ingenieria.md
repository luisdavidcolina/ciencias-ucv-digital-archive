...
| IN-217 | N+1 real en la importación CSV: hasta 8 consultas por fila, hasta 20.000 filas | `admin/imports.py:194-268` (empleados), `:342-409` (documentos) | `import_empleados_csv` hace, por cada fila del bucle `for i, row in enumerate(reader, 1)`: un `SELECT` de `existing` (:194), hasta tres `_resolve_or_create_lookup` (cargo/depto/estado, cada uno con su propio `SELECT`+`INSERT` condicional) y un `UPDATE`/`INSERT` final — hasta 5 consultas por fila. `import_documentos_csv` añade un `SELECT` de empleado por cédula (:376) y, en la rama Archivo, un `INSERT`/`ON CONFLICT` por cada palabra clave del `pk_str.split(";")` (:360-369) más el `INSERT` del documento — hasta 3 + N-palabras-clave por fila. Con `_MAX_CSV_ROWS = 20000` (imports.py:23), un CSV grande puede disparar decenas de miles de consultas en una sola petición HTTP, cada una con su propio round-trip a Neon; no hay incidente reportado porque nadie ha subido un CSV de ese tamaño todavía, pero el patrón está ahí. No es el mismo hallazgo que IN-042 (capa de repositorios) ni IN-054 (imports perezosos): ninguno de los dos menciona cantidad de consultas. | No se ejecuta: convertir esto en consultas por lotes (`WHERE cedula = ANY(%s)` para resolver todas las cédulas existentes de una vez, batch-resolve de cargo/depto/estado antes del bucle, `INSERT ... ON CONFLICT` con `execute_values`/`unnest` para filas y palabras clave) cambia la forma de la lógica fila-por-fila (que hoy decide condicionalmente qué columnas tocar según lo que trae cada fila — OR-004) y el manejo de errores por fila (`results["errors"].append(f"Fila {i}...")`) que depende de aislar cada fila en su propia transacción. Es exactamente el tipo de reforma que ya se dejó fuera por prudencia en IN-066/IN-042: plan es (1) pre-cargar en memoria todos los lookups de cargos/departamentos/estados con una consulta cada uno antes del bucle, (2) resolver todas las cédulas existentes con un solo `SELECT ... WHERE cedula = ANY(%s)`, (3) sólo entonces reservar el bucle fila-por-fila para la lógica condicional en memoria, dejando una única transacción de escritura por lotes al final. | M | `admin/imports.py` **[CHOCA]** con IN-042/IN-054 (mismo fichero, mismo ciclo de refactor) |
