  ...
  if (byStatus.length) {
    ...
    if (ctx) _chartInstances[`by-status-${suf}`] = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{ data: byStatus.map(r => r.value), backgroundColor: ["#1cc88a","#6f42c1","#e74a3b","#f6c23e","#858796"], borderWidth: 2 }]
      },
      options: { responsive: true, plugins: { legend: { position: "right" } } }
    });
  }

  // Bar horizontal: departamentos
...
async function handleImportCSV(tipo, suf) {
  ...
  try {
    ...
    const importSummary = (data.inserted!==undefined?`${data.inserted} insertados, `:'') +
    ...
    if (resultEl) resultEl.innerHTML = `
      <div class="alert alert-${data.errors?.length?'warning':'success'} p-2 mb-0">
        <strong>Importación completada:</strong> ${importSummary}
        ${errs?`<ul class="mb-0 mt-1">${errs}${moreErrs}</ul>`:''}
      </div>`;
    const lbl = fileInput.parentElement?.querySelector('.custom-file-label');
    if (lbl) lbl.innerText = fileInput.files[0].name;
  } catch(e) {
