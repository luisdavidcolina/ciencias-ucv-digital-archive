...
async function loadMonitorTable() {
  ...
  try {
    ...
    if (typeSelector && typeSelector.options.length <= 1 && state.choices) {
      ...
      typeSelector.innerHTML = `<option value="">Filtrar por TipologÃ­a...</option>` +
...
async function _renderMonitorStatusBadges() {
  ...
  try {
    ...
    const defs = [
      { key: "revision",  label: "Pendientes revisiÃ³n", cls: "badge-warning text-dark", icon: "fa-clock" },
...
function renderMonitorTable() {
  ...
  if (summaryEl) summaryEl.innerText = `Total: ${total} registros en el mÃ³dulo ${state.user.modulo}`;

  const perPage    = state.adminTable.perPage || 25;
  const totalPages = Math.ceil(total / perPage) || 1;
  const pageInfo   = document.getElementById(`admin_page_info-${suf}`);
  if (pageInfo) pageInfo.innerText = `PÃ¡g ${state.adminTable.page} de ${totalPages}`;
  ...
  if (records.length === 0) {
    container.innerHTML = `<tr><td colspan="${isArch ? 6 : 7}" class="text-muted text-center p-3">NingÃºn archivo coincide con los criterios de bÃºsqueda.</td></tr>`;
    return;
  }
  ...
  const STATUS_BADGES = {
    ...
    revision:   '<span class="badge badge-warning text-dark ds-status-badge ds-status-revision" title="Pendiente de revisiÃ³n"><i class="fas fa-clock mr-1"></i>RevisiÃ³n</span>',
  ...
  if (isArch) {
    container.innerHTML = records.map(f => {
      ...
      const autor = typeof highlightTerms === "function"
        ...
        : (f.autor || "â€”");
      const statusBtnTitle = { draft: "Borrador", revision: "En revisiÃ³n", aprobado: "Aprobado", rechazado: "Rechazado" }[f.status] || "Aprobado";
      return `<tr>
